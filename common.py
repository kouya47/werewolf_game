...
class TextDrawer:
    ...
    def draw_text_with_line_break(self, text, font_size, x, y, w,
        ...
        for character in text:
            ...
            if blit_x + await character_surface.get_rect().w >= x + w:
                blit_x = x
                blit_y += await character_surface.get_rect().h
            SCREEN.blit(character_surface, (blit_x, blit_y))
            blit_x += await character_surface.get_rect().w
...
class Button:
    ...
    def draw(self, text):
        ...
        button_rect = await self.get_rect()
        button_text_surface = font.render(text, True, self.color)
        button_text_rect = await button_text_surface.get_rect(center=(self.
